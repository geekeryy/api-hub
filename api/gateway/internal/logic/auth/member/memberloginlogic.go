...
func (l *MemberLoginLogic) MemberLogin(req *types.MemberLoginReq) (resp *types.MemberLoginResp, err error) {
	...
	token, exp, err := jwks.GenerateToken(jwksRecord.Kid, memberID, string(privateKey), l.svcCtx.Config.Auth.AccessExpire)
	if err != nil {
		l.Errorf("Failed to generate token. Error: %s", err)
		return nil, err
	}
	refreshToken, refreshExp, err := jwks.GenerateToken(jwksRecord.Kid, memberID, string(privateKey), l.svcCtx.Config.Auth.RefreshExpire)
	if err != nil {
		l.Errorf("Failed to generate token. Error: %s", err)
		return nil, err
	}
