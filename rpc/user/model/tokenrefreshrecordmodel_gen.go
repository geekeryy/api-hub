...
func (m *defaultTokenRefreshRecordModel) Update(ctx context.Context, data *TokenRefreshRecord) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, tokenRefreshRecordRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, data.RefreshTokenHash, data.Token, data.Kid, data.Ip, data.UserAgent, data.ExpiredAt, data.Id)
	return err
}
